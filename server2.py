...
def open_url(url):
    """Open a URL — uses termux-open-url on Termux, webbrowser elsewhere."""
    if TERMUX:
        try:
...
def is_apache_installed():
    ...
    if TERMUX:
        for cmd in ['apachectl', 'httpd']:
            result = subprocess.run(['which', cmd], capture_output=True, text=True)
            if result.returncode == 0:
                return True
        return False
    result = subprocess.run(['which', 'apache2'], capture_output=True, text=True)
...
def repair_cloudflared():
    ...
    def _test_run():
        try:
            if TERMUX:
                import shlex
                cmd_list = shlex.split(f'{cf_cmd} --protocol http2 tunnel --url http://127.0.0.1:80')
    ...
    if TERMUX:
        ...
        if is_apache_installed():
            ...
            try:
                ...
                subprocess.run(['pkg', 'update', '-y'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)
                subprocess.run(['pkg', 'install', '-y', 'apache2'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)
    ...
    try:
        if WINDOWS:
            ...
            if r.returncode != 0:
                ...
                slow(f'Error starting server: {err}', RED)
                input(GREEN + "\nPress Enter to return to main menu..." + RESET)
                return
...
def stop_server():
    ...
    try:
        if WINDOWS:
            ...
            r = subprocess.run(['apachectl', 'stop'], capture_output=True, text=True)
            if r.returncode != 0:
                err = r.stderr.strip() or r.stdout.strip()
                slow(f'Error stopping server: {err}', RED)
                input(GREEN + "\nPress Enter to return to main menu..." + RESET)
                return
        else:
...
def restart_server():
    ...
    try:
        if WINDOWS:
            ...
            r = subprocess.run(['apachectl', 'restart'], capture_output=True, text=True)
            if r.returncode != 0:
                err = r.stderr.strip() or r.stdout.strip()
                slow(f'Error restarting server: {err}', RED)
            else:
...
def create_cloudflared_link():
    ...
    def _run_tunnel():
        try:
            if TERMUX:
                import shlex
                cmd_list = shlex.split(f'{cloudflared_cmd} tunnel --url http://127.0.0.1:80')
                proc = subprocess.Popen(
                    cmd_list, shell=False,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
